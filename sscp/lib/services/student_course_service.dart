...
class StudentCourseService {
  ...
  Future<Map<CourseType, List<Course>>> getAvailableCoursesGroupedByType(
    ...
    try {
      ...
      for (final courseType in CourseType.values) {
        ...
        print(
            'DEBUG: Querying courses - type="$typeStr", applicableYears contains "$year", isActive=true');
        ...
        print(
            'DEBUG: Found ${snapshot.docs.length} $typeStr courses before branch filter');
        ...
        print(
            'DEBUG: Found ${courses[courseType]!.length} $typeStr courses after branch filter for "$branch"');
      }
  ...
  Future<CourseRequirement?> getCourseRequirement(String year, String branch,
      {String? semester}) async {
    try {
      print(
          'DEBUG: Querying courseRequirements - year="$year", branch="$branch", semester="$semester"');
      ...
      if (matchingDocs.isNotEmpty) {
        ...
        print(
            'DEBUG: Requirement found - OE: ${req.oeCount}, PE: ${req.peCount}, SE: ${req.seCount}');
        return req;
      }

      // DEBUG: Try to get all requirements to see what's available
      print(
          'DEBUG: No requirement found. Fetching all requirements to diagnose...');
      ...
      for (final doc in allReqs.docs) {
        final data = doc.data() as Map<String, dynamic>;
        print(
            'DEBUG: Available requirement - year="${data['year']}" (${data['year'].runtimeType}), branch="${data['branch']}", semester="${data['semester']}"');
      }
  ...
  Future<void> saveStudentSelection(StudentCourseSelection selection) async {
    try {
  ...
  Future<void> submitCourseRegistration(
    ...
    try {
      await _studentCoursesCollection.doc(selection.id).set(
            selection
                .copyWith(isSubmitted: true, updatedAt: DateTime.now())
                .toFirestore(),
            SetOptions(merge: true),
          );
    } catch (e) {
  ...
  Future<void> addCourseSelection(String studentId, String year, String branch,
      String courseId, String courseType,
      {StudentCourseSelection? currentSelection}) async {
    try {
      ...
      final currentCount =
          (selection.selectionsByType[courseType] as List<dynamic>?)?.length ??
              0;
  ...
  Future<void> removeCourseSelection(String studentId, String year,
      String branch, String courseId, String courseType,
      {StudentCourseSelection? currentSelection}) async {
    try {
  ...
  Map<String, dynamic> validateSelections(
    ...
    if (!oeValid) {
      errors.add(
          'Open Electives: Select ${requirement.oeCount} (Selected: $oeCount)');
    }
    if (!peValid) {
      errors.add(
          'Program Electives: Select ${requirement.peCount} (Selected: $peCount)');
    }
    if (!seValid) {
      errors.add(
          'Subject Electives: Select ${requirement.seCount} (Selected: $seCount)');
    }
  ...
  Map<String, Map<String, dynamic>> getValidationStatus(
    ...
    return {
      'OE': {
        ...
        'isValid':
            (selections['OE'] as List<dynamic>?)?.length == requirement.oeCount,
      },
      'PE': {
        'required': requirement.peCount,
        'selected': (selections['PE'] as List<dynamic>?)?.length ?? 0,
        'isValid':
            (selections['PE'] as List<dynamic>?)?.length == requirement.peCount,
      },
      'SE': {
        'required': requirement.seCount,
        'selected': (selections['SE'] as List<dynamic>?)?.length ?? 0,
        'isValid':
            (selections['SE'] as List<dynamic>?)?.length == requirement.seCount,
      },
    };
  }

  /// Helper method to get required count for a course type
  int _getRequiredCountForType(
      CourseRequirement requirement, String courseType) {
    switch (courseType) {
      case 'OE':
  ...
  Future<Map<SubjectType, List<Subject>>> getElectiveSubjects({
    ...
    try {
      ...
      electives[SubjectType.oe] =
          oeSnapshot.docs.map((doc) => Subject.fromFirestore(doc)).toList();
      ...
      electives[SubjectType.pe] =
          peSnapshot.docs.map((doc) => Subject.fromFirestore(doc)).toList();
  ...
  Future<void> autoRegisterCoreSubjects({
    ...
    try {
      final docId = '${studentId}_${year}_$semester';
      final docRef =
          _firestore.collection('studentSubjectSelections').doc(docId);
    ...
    try {
      ...
      for (var i = 0; i < subjectCodes.length; i += 10) {
        batches.add(subjectCodes.sublist(
            i, i + 10 > subjectCodes.length ? subjectCodes.length : i + 10));
      }
      ...
      for (final batch in batches) {
        ...
        for (final doc in snapshot.docs) {
          ...
          final assignedBatches =
              List<String>.from(data['assignedBatches'] ?? []);
